...
public class Multiply : DynamicIONode
{
    ...
    private Transition Operate()
    {
        ...
        Transition mul = InputToken.First.Type switch
        {
            TransitionType.Int => InputToken.Aggregate(Transition.One,
                (acc, current) => acc * (current.State.IsNull ? Transition.One : current.State)),
            TransitionType.Float => InputToken.Aggregate(Transition.FloatOne,
                (acc, current) => acc * (current.State.IsNull ? Transition.FloatOne : current.State)),
            _ => OutputToken.First.Type.Null()
        };

        return mul.Convert(OutputToken.First.Type);
    }
