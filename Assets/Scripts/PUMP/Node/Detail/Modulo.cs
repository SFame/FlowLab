...
public class Modulo : Node
{
    ...
    protected override string NodeDisplayName => "Mod";

    protected override float NameTextSize => 18f;

    protected override List<string> InputNames => new List<string>() { "a", "b" };

    protected override List<string> OutputNames => new List<string>() { "mod" };
    ...
    private Transition Operate()
    {
        ...
        // Divide-by-zero guard: modulo by zero is undefined -> null output.
        if (inputType == TransitionType.Int)
        {
            if ((int)InputToken[1].State == 0)
                return OutputToken.FirstType.Null();
        }
        else // Float
        {
            if (Mathf.Approximately(InputToken[1].State, 0f))
                return OutputToken.FirstType.Null();
        }

        return OutputToken.FirstType switch
        {
            TransitionType.Int when inputType == TransitionType.Int => (int)InputToken[0].State % (int)InputToken[1].State,
            TransitionType.Int when inputType == TransitionType.Float => (int)((float)InputToken[0].State % (float)InputToken[1].State),
            TransitionType.Float when inputType == TransitionType.Int => (float)((int)InputToken[0].State % (int)InputToken[1].State),
            TransitionType.Float when inputType == TransitionType.Float => (float)InputToken[0].State % (float)InputToken[1].State,
