...
public class Average : DynamicIONode, INodeAdditionalArgs<int>
{
    ...
    protected override string NodeDisplayName => "Avg";
    ...
    protected override string DefineOutputName(int tpIndex) => "avg";
    ...
    private Transition Operate()
    {
        ...
        Transition divValue = InputToken.FirstType == TransitionType.Int ? InputToken.Count : (float)InputToken.Count;
        Transition total = InputToken.Select(sf => sf.State).Aggregate((left, right) => left + right);
